...
/** Convenient unit type, in order to have a version of null with toString
  * defined so we can use it in boolean `Expr`s. See: [[SqlBoolean]].
  */
class SqlNull:
  override def toString: String = "NULL"

/** SQL really uses ternary logic: NULL behaves weirdly with the two booleans,
  * and the 3 values generate their own truth table. This is the principle on
  * which Ternary Logic Partitioning is built. It's useful to have a dedicated
  * type here.
  */
