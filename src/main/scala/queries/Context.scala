 ...
 * queries that match the ANTLR grammar, they'll never match a useful index with real fields. The context is used to
 * give the queries some room to operate in.
 *
 * @param indices A list of indices available to use, e.g. for SELECT or JOIN queries.
 */
